  ...
  <section class="d-grid gap-3">
    ...
    <article class="kb-panel">
      ...
      <form class="row g-3" @submit.prevent="handleSubmit">
        <div class="col-12">
          <label for="fixed-title" class="form-label small text-secondary"
            >항목명</label
          >
          <input
            id="fixed-title"
        ...
        <div class="col-12 col-md-8">
          <label for="fixed-amount" class="form-label small text-secondary"
            >금액</label
          >
          <input
            id="fixed-amount"
        ...
        <div class="col-12 col-md-4">
          <label for="fixed-due-day" class="form-label small text-secondary"
            >결제일</label
          >
          <select
            id="fixed-due-day"
...
const { fixedExpenseSetting, expenseCategories, transactions } =
  storeToRefs(store);
...
const fixedItems = computed(() => {
  const settings = (fixedExpenseSetting.value || []).map((item) => ({
    ...
    uid: `setting-${item.id}`,
    sourceType: "setting",
    categoryName: getCategoryById(item.categoryId)?.name || "??",
  }));
